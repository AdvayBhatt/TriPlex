...
Use `python scripts/run_pipeline.py --sample` from the repository root for the current judge workflow, and read the root README before running full data. The detailed comparison with Dhanush's separate branch is in `report/DAY1_REVIEW.md`; the current implementation and verification are documented in `report/CODE_ALIGNMENT.md`.
